...
//SPI(NRF24L01)寄存器地址
#define NRF24L01_CONFIG          0x00  //配置寄存器地址;bit0:1接收模式,0发射模式;bit1:电选择;bit2:CRC模式;bit3:CRC使能; bit4:可屏蔽中断MAXRT 0MAX_RT中断时IRQ引脚为低电平，1IRQ不产生TX_DS中断 bit5:可屏蔽中断TX_DS 同上， bit6：可屏蔽中断RX_RD 同上                              //bit4:中断MAX_RT(达到最大重发次数中断)使能;bit5:中断TX_DS使能;bit6:中断RX_DR使能
...
//SPI片选信号   CSN
#define NRF24L01_CSN_H()  GPIO_SetBits(GPIOB, GPIO_Pin_10)
#define NRF24L01_CSN_L()  GPIO_ResetBits(GPIOB, GPIO_Pin_10)

//IRQ主机数据输入  
//#define NRF24L01_IRQ  GPIO_ReadInputData(GPIOC,GPIO_Pin_5)  
//#define NRF24L01_IRQ  GPIO_ReadInputData(GPIOB,GPIO_Pin_11)  
...
u8 NRF24L01_Write_Buf(u8 reg,const u8 *pBuf, u8 u8s);//写数据区
u8 NRF24L01_Read_Buf(u8 reg, u8 *pBuf, u8 u8s);//读数据区  
u8 NRF24L01_Read_Reg(u8 reg); //读寄存器
u8 NRF24L01_Write_Reg(u8 reg, u8 value);//写寄存器
u8 NRF24L01_Check(void);//检查24L01是否存在
u8 NRF24L01_TxPacket(const u8 *txbuf);//发送一个包的数据
